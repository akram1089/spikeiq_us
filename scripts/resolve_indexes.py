"""Resolve IBKR conIds for all unresolved INDEX instruments.

Usage:
  python -m scripts.resolve_indexes
  docker exec quant_backend python -m scripts.resolve_indexes
"""
...
async def main_async() -> None:
    ...
    try:
        ...
        pending = repo.list_unresolved(limit=500, asset_types=["INDEX"])
        logger.info(f"Resolving {len(pending)} unresolved indexes...")
        for inst in pending:
    ...
    logger.success(f"Index resolution complete: resolved={resolved}, failed={failed}")
